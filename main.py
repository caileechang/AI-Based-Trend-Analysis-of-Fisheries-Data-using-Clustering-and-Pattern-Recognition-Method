...
# Make the figure a bit wider to prevent label overlap
       ...
       # else:
        ...
        if n_outliers > 0:
            ...
            st.subheader("Hierarchical Clustering (Freshwater + Marine Composition)")
        
            from scipy.cluster.hierarchy import dendrogram, linkage, fcluster
            from sklearn.preprocessing import StandardScaler
            import scipy.cluster.hierarchy as sch
        
            # --- User selects level (Yearly / Monthly) ---
            ...
            if level_choice == "Yearly":
                ...
                df_comp.rename(columns={"Freshwater": "Freshwater (Tonnes)",
                                        "Marine": "Marine (Tonnes)"}, inplace=True)
                index_col = "Year"
                ...
                df_comp.rename(columns={"Freshwater": "Freshwater (Tonnes)",
                                        "Marine": "Marine (Tonnes)"}, inplace=True)
                df_comp["MonthYear"] = pd.to_datetime(
                    df_comp["Year"].astype(int).astype(str)
                    + "-" + df_comp["Month"].astype(int).astype(str) + "-01"
                )
                index_col = "MonthYear"
        
            # --- Scale features ---
            features = ["Freshwater (Tonnes)", "Marine (Tonnes)"]
            scaled = StandardScaler().fit_transform(df_comp[features])
            ...
            # --- Perform linkage & plot dendrogram ---
            linked = linkage(scaled, method=linkage_method)
        
            fig, ax = plt.subplots(figsize=(10, 5))
            dendrogram(linked, labels=df_comp[index_col].astype(str).values, orientation="top")
            ax.set_title(f"{level_choice} Hierarchical Clustering Dendrogram ({linkage_method} linkage)")
            ax.set_xlabel("Time")
            ax.set_ylabel("Distance")
            st.pyplot(fig)
        
            # --- Optional: let user choose number of clusters to color the dendrogram ---
            num_clusters = st.slider("Select number of clusters to highlight", 2, 10, 3)
            cluster_labels = fcluster(linked, num_clusters, criterion="maxclust")
            df_comp["Cluster"] = cluster_labels
        
            st.markdown(f"**Cluster assignments (k = {num_clusters}):**")
            st.dataframe(df_comp[[index_col, "Freshwater (Tonnes)", "Marine (Tonnes)", "Cluster"]])
        
            # --- Show mean composition per cluster ---
            summary = (
                df_comp.groupby("Cluster")[["Freshwater (Tonnes)", "Marine (Tonnes)"]]
                .mean()
                .reset_index()
            )
            st.markdown("### Average Composition per Cluster")
