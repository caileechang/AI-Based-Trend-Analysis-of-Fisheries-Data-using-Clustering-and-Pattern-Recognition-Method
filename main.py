...
# Make the figure a bit wider to prevent label overlap
       # fig, ax = plt.subplots(figsize=(14, 6))
        ...
        section_header(
            ...
            "Explore freshwater & marine fish landing patterns over time using K-Means clusters.",
            icon="📊",
        )

        # ============================
        # CONTROL BAR (PERIOD + TREND)
        # ============================
        with st.container():
        ...
        if period_choice == "Yearly":
            ...
            # -------------------- SUMMARY CARDS --------------------
            st.markdown(f"### Landing Summary — {latest_year}")
            ...
            with c1:
                metric_card(
                    ...
                    fw_latest,
                    unit="tonnes",
                    delta_html=growth_html(fw_latest, fw_prev),
                    accent="#4dabf7",
                )
            with c2:
                metric_card(
                    ...
                    ma_latest,
                    unit="tonnes",
                    delta_html=growth_html(ma_latest, ma_prev),
                    accent="#ff6b6b",
                )

            st.markdown("<hr>", unsafe_allow_html=True)

            # -------------------- CLUSTERED YEARLY TRENDS --------------------
            st.markdown("### Clustered Yearly Trends")
            ...
            st.caption(f"Using optimal **k = {best_k}** (from composition-based analysis).")
            ...
            # Filter by trend choice
            if trend_option != "Both":
                melted = melted[melted["Type"].str.contains(trend_option, case=False)]

            import plotly.express as px

            color_map = {
                "Freshwater (Tonnes)": "#4dabf7",
        ...
        # =====================================================================
            ...
            # -------------------- SUMMARY CARDS --------------------
            st.markdown(f"### Landing Summary — {latest_date.strftime('%B %Y')}")
            ...
            with c1:
                metric_card(
                    ...
                    fw,
                    unit="tonnes",
                    delta_html=growth_html(fw, fw_prev),
                    accent="#4dabf7",
                )
            with c2:
                metric_card(
                    ...
                    ma,
                    unit="tonnes",
                    delta_html=growth_html(ma, ma_prev),
                    accent="#ff6b6b",
                )

            st.markdown("<hr>", unsafe_allow_html=True)

            # -------------------- CLUSTERED MONTHLY TRENDS --------------------
            st.markdown("### Clustered Monthly Trends")
