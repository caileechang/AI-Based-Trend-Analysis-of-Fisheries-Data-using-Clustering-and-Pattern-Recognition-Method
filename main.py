...
# Compute automatic bounds to include all states tightly (Peninsular + Borneo)
        ...
        # -------------------------------------------------------------------
        # EFFICIENCY HEATMAP — FIXED (NO NaN)
        # -------------------------------------------------------------------
        ...
        def section_title(title, subtitle=None):
            ...
            <hr style="border:0.5px solid #374151; margin-bottom:25px;">
            """, unsafe_allow_html=True)

        # =====================================================
        # START GEO SECTION WRAPPER (IMPORTANT)
        # =====================================================
        st.markdown('<div class="geo-section">', unsafe_allow_html=True)
        ...
        def card(style, title, value, subtitle=None):
            st.markdown(f"""
            <div style="{style}">
                <div style="font-size:15px; color:#e5e7eb;">{title}</div>
                {f"<div style='font-size:20px; font-weight:600;'>{subtitle}</div>" if subtitle else ""}
                <div style="font-size:36px; font-weight:700;">{value}</div>
            </div>
            """, unsafe_allow_html=True)

        style_blue = "background:#0f172a;padding:26px;border-radius:22px;height:170px;"
        style_green = "background:#065f46;padding:26px;border-radius:22px;height:170px;"
        style_red = "background:#7f1d1d;padding:26px;border-radius:22px;height:170px;"

        with c1: card(style_blue, "Total Landing", f"{total_land:,.0f}")
        with c2: card(style_blue, "Total Vessels", f"{total_vess:,.0f}")
        with c3: card(style_green, "Highest Landing", f"{high['Landing']:,.0f}", high["State"])
        with c4: card(style_red, "Lowest Landing", f"{low['Landing']:,.0f}", low["State"])

        # =====================================================
        # MAP
        # =====================================================
        m = folium.Map(location=[4.2, 108.5], zoom_start=6, tiles="CartoDB dark_matter")

        HeatMap(
            [[lat, lon, val] for (lat, lon), val in zip(df_year["Coords"], df_year["Landing"])],
            radius=35, blur=22
        ).add_to(m)
